...
class Piloted extends EventEmitter {
  ...
  config (config, callback) {
    ...
    this._consul = new Consulite({ consul: this._config.consul });

    this._services = this._config.watches ? this._config.watches.map((watch) => {
      return watch.name;
    }) : [];

    Items.parallel(this._services, (service, next) => {
      this._consul.getService(service, next);
    }, callback);
  ...
  service (service) {
    Hoek.assert(this._services.indexOf(service) !== -1, `${service} isn't configured to be watched`);

    return this._consul.getCachedService(service);
  }

  serviceHosts (service) {
    Hoek.assert(this._services.indexOf(service) !== -1, `${service} isn't configured to be watched`);

    return this._consul.getCachedServiceHosts(service);
  }
  ...
  refresh () {
    ...
    Items.parallel(this._services, (service, next) => {
      this._consul.refreshService(service, next);
    }, () => {
      this.emit('refresh');
...
internals.template = function (config) {
  const str = (typeof config === 'string') ? config : JSON5.stringify(config);
